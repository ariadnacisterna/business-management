...
class MovementReason(Base, AuditedMixin):
    __tablename__ = "movement_reason"
    __table_args__ = (
        status_check_constraint(),
        UniqueConstraint("business_id", "name", name="uq_movement_reason_business_id_name"),
    )
    ...
    business: Mapped["Business"] = relationship(back_populates="movement_reasons")
    stock_movements: Mapped[list["StockMovement"]] = relationship(back_populates="reason")
