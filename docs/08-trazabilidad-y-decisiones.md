...
| D-004 | Cada persona tendrá una cuenta individual con rol Administradora o Empleada. | Permite aplicar permisos y atribuir operaciones correctamente. |
...
| D-028 | La recuperación de una contraseña será un restablecimiento realizado por una Administradora, sin correo electrónico. | Evita incorporar infraestructura de correo y datos personales adicionales en un sistema privado de pocos usuarios. Resuelve DP-004; queda pendiente el procedimiento restringido para el caso sin ninguna Administradora disponible. |
...
| D-032 | Ninguna conversación de IA ejecuta `git commit`, `git merge` ni `git push`; esos pasos los realiza siempre el Responsable del proyecto. Las conversaciones dejan cambios en stage y sugieren el mensaje de commit en formato Conventional Commits. Ver [10-flujo-de-trabajo.md](10-flujo-de-trabajo.md#los-commits-merges-y-push-son-siempre-manuales). | El Responsable del proyecto quiere retener el control manual sobre qué queda escrito en el historial de git, sin excepciones caso a caso. |
...
Estas dos decisiones cambiaron de contenido al incorporarse los objetivos de escaneo y de adaptación a otros rubros. Se conserva la formulación anterior para que la revisión del documento sea trazable.
...
| DP-003 | ¿La importación debe ser total o permitir aplicar solo filas válidas? | Probar una planilla real y elegir la opción menos riesgosa. |
...
| DP-009 | ¿Qué procedimiento restringido permite recuperar el acceso si no queda ninguna Administradora disponible? | Definir un procedimiento operativo, fuera de la aplicación normal, antes de usar datos reales. No bloquea el inicio de la programación. |
...
Con esa muestra se podrán cerrar DP-001 a DP-003 y DP-006, y diseñar la plantilla de importación.
