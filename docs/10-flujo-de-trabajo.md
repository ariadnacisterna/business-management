...
- **Conversación madre:** sostiene el estado del proyecto. Aprueba decisiones,
  revisa el trabajo terminado y decide si se integra a `main`. No implementa
  tareas grandes por sí misma.
- **Conversación de tarea:** conversación nueva y acotada a una sola tarea.
  ...
  dejando una rama de git lista para revisar.
...
- Cada tarea vive en su propia rama: `tarea/<id>-<descripcion-corta>`, por
  ejemplo `tarea/T-003-alta-de-producto`.
- Una conversación de tarea **nunca** hace merge a `main` por su cuenta.
- Al terminar, deja: los commits en su rama, y su fila actualizada en el
...
Instrucciones:
1. Creá la rama tarea/T-XXX-<descripcion> desde main.
2. Implementá solo lo necesario para esta tarea.
3. Agregá pruebas cuando corresponda.
4. Commiteá con mensajes claros.
5. No hagas merge a main.
6. Al terminar, actualizá tu fila en la tabla de docs/10-flujo-de-trabajo.md
   con estado "Lista para revisión" y un resumen breve de qué archivos
   tocaste y qué quedó pendiente, si algo.
...
6. Si está bien: mergear a `main` y marcar la tarea `Verificada` con la fecha.
