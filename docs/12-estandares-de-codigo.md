...
4. Que `npm run lint` (frontend) no muestre errores ni avisos. La carga de
   datos de una pantalla usa el hook `shared/useLoad.ts`
   (`const { status, data, reload, setData } = useLoad(fetcher, deps)`) en vez
   de `useEffect(load, [...])` con `setStatus('loading')` dentro; el
   reinicio de estado derivado (página, importe, aviso) se hace en los
   manejadores o derivándolo, nunca con `setState` síncrono dentro de un
   `useEffect`.
